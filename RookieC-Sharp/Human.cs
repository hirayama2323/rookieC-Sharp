...
namespace RookieC_Sharp
{
    ...
    public class Human
    {
        ...
        public int[] putinput(string[,] bord, int number)
        {
            var socket = new SocketClass();
            // ToDo サーバー側の先攻の処理
            if (number == 1 && this.Rows == Rows.Circle)
            {
                string str;
                int[] result;
            ...
            // ToDo クライアント側の先攻の処理
            if(number == 2 && this.Rows == Rows.Circle)
            {
                string str = socket.Server(string.Empty);
                int[] result;
            ...
            // ToDo サーバー側の後攻の処理
            if (number == 2 && this.Rows == Rows.Cross)
            {
                string str;
                int[] result;
                Console.WriteLine($"{Name}さんの番です。");
            ...
            // ToDo クライアント側の後攻の処理
            if (number == 1 && this.Rows == Rows.Cross)
            {
                string str = socket.Server(string.Empty);
                int[] result;
